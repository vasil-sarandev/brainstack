# Technologies

#meta

Tool-specific reference — frameworks, databases, cloud services, and dev tooling. Linked from [topic notes](../topics/topics.md) where relevant; not duplicated there.

---
## Backend & data

- [ExpressJS](express/express.md)
- [PostgreSQL](postgresql/postgresql.md)
- [MongoDB](mongodb/mongodb.md)
- [Redis](redis/redis.md)
- [TypeORM](typeorm/typeorm.md)
- [Apache Kafka](apache-kafka/apache-kafka.md)

---
## Frontend

- [React](react/react.md)
- [Redux & Redux Toolkit](redux-rtk/redux-rtk.md)
- [NextJS](nextjs/nextjs.md)

---
## Platform & cloud

- [Docker](docker/docker.md)
- [Kubernetes](kubernetes/kubernetes.md)
- [GitHub Actions](github-actions/github-actions.md)
- [Amazon Web Services (AWS)](aws/aws.md)

---
## Tooling & monorepo

- [Turborepo](turborepo/turborepo.md)
- [Cursor](cursor/cursor.md)
- [Claude Code](claude-code/claude-code.md)

---
